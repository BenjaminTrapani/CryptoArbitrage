...
public class ArbitrageExecutor implements OrderGraphAnalysisHandler {
	...
	protected static class ExecutableTrade {
		...
		public ExecutableTrade(IntermediateTrade intTrade, Fraction sourceQuantity) {
			...
			// TODO add a check here that ensures that this.quantity is also
			// less than or equal
			// to available balance on the exchange. Cache this value in
			// IntermediateTrade or something
			if (this.quantity.compareTo(intTrade.graphEdge.quantity) > 0) {
				throw new IllegalStateException(
						"Executable quantity computed from source cannot exceed initial quantity available for trade");
			}
	...
	protected static class IntermediateTrade {
		...
		Fraction concatenatedRatioUsedForQuantAdjust; // same as
														// concatenatedRatio for
														// buy, and is
														// concatenatedRatio of
														// prev ratio for sell
		Fraction quantityInPathSourceUnits; // source quantity in terms of path
											// source units
	...
	protected static List<IntermediateTrade> computeQuantitiesInSourceUnits(
			List<IntermediateTrade> intermediateTrades) {
		for (int i = 0; i < intermediateTrades.size(); ++i) {
			IntermediateTrade intTrade = intermediateTrades.get(i);
			intTrade.concatenatedRatioUsedForQuantAdjust = intTrade.graphEdge.isBuy ? intTrade.concatenatedRatio
					: intTrade.prevConcatenatedRatio;
	...
	protected static Fraction computeLoopMaxflowInSource(List<IntermediateTrade> intTrades, CurrencyBalanceDS ds) {
		Fraction minEdgeCapacityInPathSourceUnits = null;
		for (IntermediateTrade intTrade : intTrades) {
			Fraction sourceQuantityAvailableForTrade = ds.getBalance(intTrade.sourceCurrency,
					intTrade.graphEdge.exchangeName);
			Fraction sourceQuantityAvailableForTradeInPathSourceUnits = sourceQuantityAvailableForTrade
					.divide(intTrade.prevConcatenatedRatio);
			Fraction minSourceQuantityInPathSourceUnits = intTrade.quantityInPathSourceUnits
					.min(sourceQuantityAvailableForTradeInPathSourceUnits);
			if (minEdgeCapacityInPathSourceUnits == null
					|| minEdgeCapacityInPathSourceUnits.compareTo(minSourceQuantityInPathSourceUnits) > 0) {
	...
	static List<ExecutableTrade> convertToExecutableTrades(List<IntermediateTrade> intTrades, Fraction sourceQuantity) {
	...
	List<ExecutableTrade> buildExecutableTrades(HashSet<TwoSidedGraphEdge> graphEdges, CurrencyBalanceDS ds) {
		...
		// Since the path forms a loop, ok to pick any source currency along the
		// path as
	...
	private void placeOrders(List<ExecutableTrade> trades) {
		for (ExecutableTrade trade : trades) {
			...
			trade.exchangeOrderID = subsetForTrade.placeLimitOrder(new CurrencyPair(trade.base, trade.counter),
					trade.isBuy, trade.price, trade.quantity, String.valueOf(orderID++));
	...
	public void onOrderBookAnalysisComplete(AnalysisResult analysisResult) {
		if (analysisResult.maxRatio.compareTo(minAcceptableRatio) > 0) {
			...
			System.out.println(
					"Profitable trade found with ratio " + analysisResult.maxRatio.toString() + "\n" + allTrades);
			...
			System.out.println("No profitable path found");
		}
	}
}
